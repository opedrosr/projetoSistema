...
type PaymentProfile = Profile & { pix_key?: string | null };
...
function PublicPage({ slug }: { slug: string }) {
  ...
  const profile = data?.profile as PaymentProfile | undefined;
  ...
  if (confirmed) {
    const requiresDeposit = Boolean(selected?.requires_deposit);
    const depositAmount = Number(selected?.deposit_amount || 0);
                ...
                >
                  <div>
                    ...
                    <span className="service-duration">
                      ...
                      {(service as PaymentService).requires_deposit && <> · Sinal de {formatCurrency(Number((service as PaymentService).deposit_amount || 0))}</>}
                    </span>
                  </div>
...
function BookingDrawer({
  ...
  profile: Profile;
  selected: PaymentService | null;
  setSelected: React.Dispatch<React.SetStateAction<PaymentService | null>>;
  step: number;
      ...
      <div className="booking-panel">
        ...
        {step === 3 && (
          ...
          >
            ...
            {selected && selected.requires_deposit && <div className="booking-deposit-note"><span>Sinal para reservar</span><strong>{formatCurrency(Number(selected.deposit_amount || 0))}</strong><p>Após reservar, você verá a chave Pix da profissional e poderá enviar o comprovante pelo WhatsApp.</p></div>}
...
function Services({
  ...
  const [form, setForm] = useState({ name: '', description: '', price: '', duration_minutes: '60', requires_deposit: false, deposit_amount: '' });
  const [editing, setEditing] = useState<string | null>(null);
  const paymentServices = data.services as PaymentService[];
  const resetForm = () => { setForm({ name: '', description: '', price: '', duration_minutes: '60', requires_deposit: false, deposit_amount: '' }); setEditing(null); };

  async function save(event: FormEvent) {
    event.preventDefault();
    const price = Number(form.price), deposit = Number(form.deposit_amount);
    if (!form.name.trim() || !Number.isFinite(price) || price < 0) return;
    if (form.requires_deposit && (!Number.isFinite(deposit) || deposit <= 0)) { window.alert('Informe um valor de sinal maior que zero.'); return; }
    if (form.requires_deposit && deposit > price) { window.alert('O sinal não pode ser maior que o preço do serviço.'); return; }
    const payload = { name: form.name.trim(), description: form.description.trim(), price, duration_minutes: Number(form.duration_minutes), profile_id: data.profile.id, requires_deposit: form.requires_deposit, deposit_amount: form.requires_deposit ? deposit : null };
    const result = editing ? await supabase.from('services').update(payload).eq('id', editing).select().maybeSingle() : await supabase.from('services').insert(payload).select().maybeSingle();
    if (result.error) { window.alert(`Não foi possível salvar o serviço: ${result.error.message}`); return; }
    if (result.data) { setData({ ...data, services: editing ? data.services.map(item => item.id === editing ? result.data as Service : item) : [...data.services, result.data as Service] }); resetForm(); }
  }
  ...
  return <>
    <div className="page-title"><div><div className="eyebrow">O que você oferece</div><h1>Serviços</h1><p>Mostre suas opções de forma clara para suas clientes.</p></div></div>
    <div className="two-column">
      <section className="dashboard-section"><div className="section-heading"><h2>Seus serviços</h2><span>{paymentServices.length} cadastrados</span></div>
        {paymentServices.length ? <div className="service-admin-list">{paymentServices.map(service => <div className={`service-admin ${!service.is_active ? 'inactive' : ''}`} key={service.id}>
          <div><strong>{service.name}</strong><span>{formatCurrency(service.price)} · {formatDuration(service.duration_minutes)}{service.requires_deposit ? ` · Sinal de ${formatCurrency(Number(service.deposit_amount || 0))}` : ''}</span></div>
          <div><button className="small-action" onClick={() => { setEditing(service.id); setForm({ name: service.name, description: service.description || '', price: String(service.price), duration_minutes: String(service.duration_minutes), requires_deposit: Boolean(service.requires_deposit), deposit_amount: service.deposit_amount != null ? String(service.deposit_amount) : '' }); }}>Editar</button><button className="small-action danger" onClick={() => remove(service.id)}><Trash2 size={15} /></button></div>
        </div>)}</div> : <Empty title="Nenhum serviço cadastrado" text="Adicione seu primeiro serviço para começar." />}
      </section>
      <form className="form-card" onSubmit={save}>
        <div className="section-heading"><h2>{editing ? 'Editar serviço' : 'Novo serviço'}</h2>{editing && <button type="button" className="text-link" onClick={resetForm}>Cancelar</button>}</div>
        <Field label="Nome do serviço" placeholder="Ex.: Corte e finalização" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required />
        <label className="field"><span>Descrição <em>Opcional</em></span><textarea placeholder="Descreva brevemente o serviço" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} /></label>
        <div className="form-row"><Field label="Preço" type="number" min="0" step="0.01" placeholder="0,00" value={form.price} onChange={e => setForm({ ...form, price: e.target.value })} required /><label className="field"><span>Duração</span><select value={form.duration_minutes} onChange={e => setForm({ ...form, duration_minutes: e.target.value })}><option value="30">30 minutos</option><option value="60">1 hora</option><option value="90">1h30</option><option value="120">2 horas</option><option value="150">2h30</option><option value="180">3 horas</option></select></label></div>
        <div className="service-payment-settings"><div className="service-payment-heading"><div><span className="eyebrow">Pagamento</span><h3>Sinal para reservar</h3></div><button type="button" className={`toggle ${form.requires_deposit ? 'on' : ''}`} onClick={() => setForm({ ...form, requires_deposit: !form.requires_deposit, deposit_amount: !form.requires_deposit ? form.deposit_amount : '' })}><i /></button></div><p>{form.requires_deposit ? 'A cliente deverá pagar este valor via Pix para reservar.' : 'A cliente paga diretamente no atendimento.'}</p>{form.requires_deposit && <Field label="Valor do sinal" type="number" min="0.01" step="0.01" placeholder="0,00" value={form.deposit_amount} onChange={e => setForm({ ...form, deposit_amount: e.target.value })} required />}</div>
        <Button type="submit">{editing ? 'Salvar alterações' : <><Plus size={17} /> Adicionar serviço</>}</Button>
      </form>
    </div>
  </>;
}
...
function ProfileSettings({
  ...
  const [form, setForm] = useState(data.profile as PaymentProfile);
    ...
    <>
      ...
      <div className="profile-settings-grid">
        <form className="form-card" onSubmit={save}>
          ...
          <Field label="Chave Pix" placeholder="CPF, celular, e-mail ou chave aleatória" value={form.pix_key || ''} onChange={event => setForm({ ...form, pix_key: event.target.value })} />
...
const paymentStyles = `.service-payment-settings{margin:4px 0 18px;padding:16px;border:1px solid var(--border);border-radius:8px;background:var(--surface-muted,#f8f8f7)}.service-payment-heading{display:flex;align-items:center;justify-content:space-between;gap:16px}.service-payment-settings h3{margin:3px 0 0;font-size:15px}.service-payment-settings p{margin:8px 0 0;color:var(--muted);font-size:13px;line-height:1.5}.field-help{margin-top:-8px;margin-bottom:16px;color:var(--muted);font-size:12px;line-height:1.5}.booking-deposit-note{padding:14px 15px;margin-bottom:14px;border:1px solid var(--border);border-radius:8px;background:var(--surface-muted,#f8f8f7)}.booking-deposit-note span{display:block;font-size:12px;color:var(--muted)}.booking-deposit-note strong{display:block;margin-top:3px;font-size:18px}.booking-deposit-note p{margin:6px 0 0;font-size:12px;color:var(--muted);line-height:1.45}.pix-payment{width:100%;max-width:520px;margin:20px 0;padding:20px;border:1px solid var(--border);border-radius:10px;background:var(--surface-muted,#f8f8f7);text-align:left}.pix-payment-top{display:flex;align-items:flex-start;justify-content:space-between;gap:16px}.pix-payment-top h2{margin:4px 0 0;font-size:28px}.pix-badge{padding:5px 9px;border-radius:6px;background:#dff5e8;color:#187343;font-size:12px;font-weight:700}.pix-payment>p{font-size:13px;color:var(--muted);line-height:1.5}.pix-key-box{display:flex;align-items:center;gap:10px;padding:11px 12px;border:1px solid var(--border);border-radius:7px;background:var(--surface,#fff)}.pix-key-box span{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:13px}.pix-key-box button{display:inline-flex;align-items:center;gap:6px;border:0;background:transparent;font-weight:600;cursor:pointer}.pix-missing{padding:12px;border-radius:7px;background:#fff3e6;color:#8a4a00;font-size:13px;line-height:1.45}`;
function PaymentStyles(){useEffect(()=>{const style=document.createElement('style');style.textContent=paymentStyles;document.head.appendChild(style);return()=>style.remove()},[]);return null}

function App() {
  return (<><PaymentStyles /><AppWithPaymentStyles /></>);
}

function AppWithPaymentStyles() {
